...
import { getTrustedHeaderAuthRuntimeConfig, prependBasePath } from '@/api/client'
...
export function handleApiError(error: unknown): void {
  ...
  if (status === 401) {
    if (isAccountDisabledError(error)) {
      window.location.href = prependBasePath(`/login?reason=${ACCOUNT_DISABLED_REASON}`)
      return
    }
    ...
    window.location.href = prependBasePath('/login')
    return
  }
