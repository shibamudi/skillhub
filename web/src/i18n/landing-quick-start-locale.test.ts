...
describe('landing quick start locales', () => {
  ...
  it('exposes CLI install command in both locales', () => {
    ...
    expect(zh.landing.quickStart.cli.description).toBe('安装 SkillHub CLI 到本地，后续使用 skillhub install <skill> --registry <url> 安装技能')
    expect(en.landing.quickStart.tabs.cli).toBe('CLI')
    expect(en.landing.quickStart.cli.command).toBe('npm i -g @astron-team/skillhub')
    expect(en.landing.quickStart.cli.description).toBe('Install SkillHub CLI locally, then use skillhub install <skill> --registry <url> to install skills')
  })
})
